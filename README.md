...
При `ONAPPINSTALL` приложение идемпотентно синхронизирует CRM robots: для REST-запроса
и ожидания поля регистрируется ровно по одному роботу. Существующие роботы обновляются,
а отсутствующие добавляются. Устаревшие дублирующие activities этих двух действий
удаляются, если они были зарегистрированы ранее. Роботы используют подписку и общие
